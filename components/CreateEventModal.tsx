...
const CreateEventModal: React.FC<CreateEventModalProps> = ({ onClose, onCreateEvent, user }) => {
  ...
  const [ticketTiers, setTicketTiers] = useState<TicketTierInput[]>([
    createEmptyTier('General Admission'),
  ]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const payload: Omit<Event, 'id' | 'organizer'> = {
        ...
        <header className="flex-shrink-0 flex items-center justify-between px-6 py-4 border-b bg-gray-50">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Craft your experience, add ticket tiers, and drop a pin so attendees can find you.
            </p>
          </div>
          <button
            type="button"
